...
def draw_markers(frame, results, bounding_boxes, scale_factor, mean,
    ...
    for i in results.flatten():
        ...
        if frame_ts.shape[:1] == (0,) or frame_ts.shape[1:2] == (0,):
            ...
            # print(f"class_numbers[{i}] = {class_numbers[i]}") # TODO
            # print(f"colors = {colors[class_numbers[i]]}")
            color_box_current = colors[0].tolist()
...
def main(args):
    ...
    # model = torch.hub.load('yolov5/weight', 'yolov5l_saved_model', classes=59)
    # model.load_state_dict(torch.load('yolov5l.pt')['model'].state_dict())
    ...
    # model_path = os.path.abspath("model/saved_model.pb")
    ...
    colors = np.random.randint(0, 255, size=(len(labels)+1, 3), dtype='uint8')
